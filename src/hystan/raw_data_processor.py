...
def process_csv(file_path: str) -> Optional[Tuple[Dict[str, float], pd.DataFrame]]:
    ...
    # H(kOe)カラムの追加
    data_df = data_df.copy()  # SettingWithCopyWarning対策でDataFrame のスライスを新しい DataFrame にする
...
def process_csv_and_visualize(file_path: str) -> Optional[Tuple[Dict[str, float], plt.Figure]]:
    """
